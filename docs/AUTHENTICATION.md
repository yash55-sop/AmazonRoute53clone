...
The demo uses engineered server-side sessions rather than AWS IAM. Login verifies the user's salted scrypt password, creates a cryptographically random token, stores its SHA-256 digest in SQLite, and sends the raw token only in an HTTP-only, SameSite=Lax cookie. Cookie security and lifetime are configured by environment.
